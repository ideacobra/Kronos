...
## 🤖 memecoin_bot: Solana Memecoin Paper-Trading Demo

[`memecoin_bot/`](memecoin_bot/README.md) is a **paper-trading only** demo built on top of Kronos: it reads public Solana DEX data (DexScreener, GeckoTerminal), scores tokens for short-term FOMO/momentum, optionally asks Kronos for a directional confirmation, and manages a fully simulated portfolio with real risk controls (stop-loss, take-profit, circuit breaker). It can optionally post transparent, factual trade logs to X (Twitter) -- never hype, and off by default. It never holds a real wallet key or executes a real trade. See [`memecoin_bot/README.md`](memecoin_bot/README.md) for setup, configuration, and safety details.
