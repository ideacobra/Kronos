...
## 🤖 memecoin_bot: Solana Memecoin Trading Bot

[`memecoin_bot/`](memecoin_bot/README.md) is a Solana memecoin FOMO/momentum bot built on top of Kronos: it reads public Solana DEX data (DexScreener, GeckoTerminal), scores tokens for short-term FOMO/momentum, optionally asks Kronos for a directional confirmation, and manages a portfolio with real risk controls (stop-loss, take-profit, circuit breaker). It can optionally post transparent, factual trade logs to X (Twitter) -- never hype, and off by default. **Paper trading (fully simulated, no real funds) is the default**; real live trading on Solana via Jupiter is also supported but is entirely opt-in, gated behind an explicit wallet setup and a CLI risk-acknowledgement flag required on every run. See [`memecoin_bot/README.md`](memecoin_bot/README.md) for setup, configuration, and full safety details.
